...
<body>
    ...
    if (isset($_GET['id_equipo'])) {
        ...
        $idClientes = obtenerOpciones('IDCLIENTE', 'cliente');
        ...
        $clienteActual = $row['IDCLIENTE'];
    ?>
        <a href="../read/crudequipos.php" class="btn btn-primary">
            ...
            <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                ...
                <div id="pantalla1">
                    ...
                    <select name="idCliente">
                        <option value="<?php echo $clienteActual ?>" selected>
                            <?php
                            //obtener nombre cliente Actual
                            $sqlNombreClienteActual = "SELECT NOMBRECLIENTE FROM cliente WHERE IDCLIENTE = '$clienteActual' ;";
                            $resultClienteActual = $conn->query($sqlNombreClienteActual);
                            if ($resultClienteActual->num_rows > 0) {
                                $rowClienteActual = $resultClienteActual->fetch_assoc();
                            }
                            //imprimir nombres :p
                            echo  $rowClienteActual['NOMBRECLIENTE'];
                            ?>
                        </option>
                        <?php foreach ($idClientes as $idCliente) : ?>
                            <option value="<?php echo $idCliente; ?>">
                                <?php
                                //obtener nombre de Clientes en General por row $idCliente
                                $sqlNombreCliente = "SELECT NOMBRECLIENTE FROM cliente WHERE IDCLIENTE = '$idCliente' ;";
                                $resultNombre = $conn->query($sqlNombreCliente);
                                if ($resultNombre->num_rows > 0) {
                                    $rowNombre = $resultNombre->fetch_assoc();
                                }
                                // imprimir nombres :p
                                echo $rowNombre['NOMBRECLIENTE'];
                                ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
